...
// ─── Check if a host compiler/runtime is installed ───────────────────────────
function isCommandAvailable(command) {
  try {
    execSync(`where ${command}`, { stdio: "pipe", timeout: 2000 });
    return true;
  } catch {
    return false;
  }
...
function getLanguageStatus() {
  ...
  const dockerRunning = isDockerDaemonRunning();
  const status = {};

  for (const [key, lang] of Object.entries(LANGUAGES)) {
    const cmd = compilerMap[key];
    const hostAvail = isCommandAvailable(cmd);

    status[key] = {
      name: lang.name,
      available: dockerRunning || hostAvail,
      mode: dockerRunning ? 'docker' : (hostAvail ? 'native' : 'unavailable'),
      compiler: cmd,
    };
  }
  return status;
...
module.exports = { LANGUAGES, getLanguageStatus, isCommandAvailable, isDockerDaemonRunning };
